...
### [x] 56. Bug: Unpair from parent doesn't clear active restrictions on child — 2026-03-25
`child:reset` in `index.tsx` now calls `setPolicy('')` + `dismissAllOverlays()` before navigating to `/setup`. Added `dismissAll()` static to `AppBlockerModule` and `dismissAllOverlays()` ReactMethod to `UsageStatsModule`.

### [x] 57. Bug: Overlay persists over Home screen after dismissing blocked app — 2026-03-25
Added `isCurrentHomeLauncher()` to `AppBlockerModule` using `ACTION_MAIN / CATEGORY_HOME` intent resolution. `onAccessibilityEvent` now always calls `dismissOverlay()` when the home launcher comes to foreground, bypassing the system-overlay skip guard. Also reduced `DISMISS_COOLDOWN_MS` from 2000ms to 800ms to prevent the cooldown from blocking the overlay on the user's next open of a blocked app.
