...
export default function CoparentInviteCard({ childPublicKey, childDisplayName, onConnected, onDismiss }) {
  ...
  useEffect(() => {
    window.callBare('coparent:generateInvite', { childPublicKey })
      .then(setInvite)
      .catch(() => setError('Failed to generate invite. Please try again.'));

    const unsub = window.onBareEvent('coparent:joined', (data) => {
      onConnected(data);
    });
    return unsub;
  }, [childPublicKey, onConnected]);
  ...
  return (
    <div style={cardStyle}>
      ...
      <p style={{ ...typography.caption, color: colors.text.secondary, margin: 0, marginBottom: `${spacing.base}px` }}>
        Share this with another parent to co-manage {childDisplayName}'s device.
      </p>
      ...
      <p style={{ ...typography.caption, color: colors.text.muted, fontStyle: 'italic', margin: 0, textAlign: 'center' }}>
        Waiting for co-parent to connect...
      </p>
    </div>
  );
}
