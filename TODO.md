...
| 144 | Parent/child not connected on app load | After app start, devices show disconnected; have to re-run pairing flow before they reconnect. Expected: existing paired peers auto-reconnect via Hyperswarm on launch. |
| 146 | "Device has not checked in" notifications for unpaired child | Parent receives check-in alerts for a child that is no longer paired. Stale peer record should be cleaned up, or notifications should only fire for currently-paired children. |
...
| 145 | Remove all copy/paste invite link UI | Keep only QR code pairing for now. Remove paste-link toggle and input from child-setup and anywhere else invite links are shown for manual entry. |
...
| 112 | Port screenshot automation from PearCal | Copy `scripts/android-screenshots.sh`, `scripts/ios-screenshots.sh`, `scripts/screenshots.sh`, `scripts/add-ios-sources.rb`, `scripts/frame-android-screenshots.sh`, `scripts/assets/pixel5-frame.png`. Copy `ScreenshotModule.{kt,swift,.m}` + `ScreenshotPackage.kt` (rename package/module to `PearGuardScreenshot`, register in MainApplication.kt and pbxproj). Create `src/ui/screenshot-fixtures.js` with app-specific scenes. Update `app/index.tsx` `NativeModules.PearGuardScreenshot` lookup. Set `IOS_SCREENSHOT_DEVICES`, `ANDROID_APP_ID`, `ANDROID_MAIN_ACTIVITY`, `ANDROID_SCREENSHOT_AVDS` in scripts/app.conf. The `android-screenshots.sh` script auto-invokes `frame-android-screenshots.sh` at the end, producing `metadata/android/screenshots/<avd>_Framed/`. Reference framed paths in `zapstore.yaml` `images:` and README. See p2p-wiki `concepts/screenshot-automation-pattern.md`. |
