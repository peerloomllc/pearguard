...
- **Reuses the existing brute-force lockout.** Wrong attempts feed the same persisted counter and ladder as the overlay keypad (`PIN_FREE_ATTEMPTS = 5`, then 30 s / 2 min / 10 min / 1 h, surviving force-stop, reboot and a moved clock). A child guessing at the leave screen is throttled identically to one guessing at a block screen.
