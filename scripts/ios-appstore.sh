...
if $USE_ASC; then
  ...
  # Apple's object-storage endpoints regularly drop long multipart uploads
  # mid-body ("write: broken pipe"). asc retries individual parts internally but
  # gives up after 4, so wrap the whole upload. A dropped connection should not
  # strand a release that has already been tagged and published.
  UPLOAD_ATTEMPTS="${UPLOAD_ATTEMPTS:-5}"
  UPLOAD_BACKOFF="${UPLOAD_BACKOFF:-15}"

  _upload_log=$(mktemp "${TMPDIR:-/tmp}/asc-upload.XXXXXX")
  trap 'rm -f "$_upload_log"' EXIT
  ...
  while :; do
    ...
    # A build with this version and build number already exists upstream. No
    # number of retries will change that, so fail now instead of sleeping
    # through the whole backoff schedule first.
    if grep -qiE 'already exists|duplicate|redundant binary' "$_upload_log"; then
      echo "Upload rejected as a duplicate build. Not retrying."
      exit 1
    fi

    if [ "$_attempt" -ge "$UPLOAD_ATTEMPTS" ]; then
      echo "Upload failed after ${UPLOAD_ATTEMPTS} attempts."
      exit 1
    fi

    echo "Upload attempt ${_attempt}/${UPLOAD_ATTEMPTS} failed. Retrying in ${UPLOAD_BACKOFF}s..."
    sleep "$UPLOAD_BACKOFF"
    _attempt=$((_attempt + 1))
    UPLOAD_BACKOFF=$((UPLOAD_BACKOFF * 2))
  done
else
  xcrun altool \
    --upload-app \
